...
if ($count != 0) {
  ...
  <input class="form-control" id="usersInGroupSearchInput" type="text" placeholder="Search..">
  <br>
  ...
  while ($row = fetch_array($result)) {
    ...
    $("#usersNotInGroupRow_'.$Id.'").click(function(){
      $.ajax({
        type: "POST",
        url: "functions/groups/removeUser.php",
        data: {
          UserId:"'.$Id.'",
          IdGroup:"'.$IdGroup.'"
        },
...
$(document).ready(function(){
  $("#usersInGroupSearchInput").on("keyup", function() {
    var value = $(this).val().toLowerCase();
    $("#usersInGroupTable tr").filter(function() {
